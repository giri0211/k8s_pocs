...
output "secrets" {
  sensitive = false
  # jsondecode(data.aws_secretsmanager_secret_version.loki_secrets[0].secret_string)[local.loki_secret_name]
  value = jsondecode(data.aws_secretsmanager_secret_version.loki_secrets[0].secret_string)["LOKI_TENANT_k8s-app-logs"]
}
