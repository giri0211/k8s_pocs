...
aws iam list-roles --query 'Roles[?RoleName==`integrations-onboarding-b-support-admin-access`].Arn'
...
aws eks --region us-east-1 update-kubeconfig --name integrations-onboarding-a 
aws sts assume-role --role-arn arn:aws:iam::545444110299:role/integrations-onboarding-a-support-admin-access --role-session-name cluster-admin1
aws eks --region us-east-1 update-kubeconfig --name integrations-onboarding-a  --role-arn arn:aws:iam::545444110299:role/integrations-onboarding-a-support-admin-access

# namespace access role csa-deployments-project-template-access
pie-deployments-green

# Assume the namespace access role csa-deployments-project-template-access
aws sts assume-role --role-arn arn:aws:iam::545444110299:role/pie-deployments-green-platform-showcase-access --role-session-name ns-access
# Update kubeconfig with cluster administrator role
...
# Assume the namespace access role csa-deployments-project-template-access
aws sts assume-role --role-arn arn:aws:iam::545444110299:role/csa-deployments-project-template-access --role-session-name ns-access
# Update kubeconfig with cluster administrator role
